...
#include <string.h> 
#include <unistd.h> // for uusleep func

#include "gen_func.c"

#define MAX_MEMBERS 5
#define MAX_ROWLN 1010 // for csv lines
#define MAX_STRLN 101 // 100 with null
...
double jogging_recommend(double user_pace);
void choices(struct jogger_info j[MAX_MEMBERS], int jogger_index);
// screens in order
void splash_screen();
    ...
    void jog_squad_members();
        ...
        void log_in();
            ...
            void access_jogger(struct jogger_info j[MAX_MEMBERS], int jogger_index);
                void view_profile(struct jogger_info j, struct jogger_info z[MAX_MEMBERS], int jogger_index);
                void log_jogging(struct jogger_info j, struct jogger_info z[MAX_MEMBERS], int jogger_index);
        ...
        void sign_up();
            ...
            int csvrow_read_signup(const char *csv_file, struct jogger_info j[MAX_MEMBERS]);
                void csvrow_read_jogentry(const char *csv_file, struct jogger_info j[MAX_MEMBERS]); // read excel, store in array
                // array is updated
                void clear_csv(const char *csv_file); // clear everything
                // ignore header row for printing
...
void splash_screen() {
    ...
    display_ascii("ascii_art/jog_squad.txt", 0);
    // display_ascii("ascii_art/subhead_js.txt", 500);
    
    usleep(1500); // 2 seconds
}
...
void jog_squad_members() {
    ...
    int limit_checker = csvrow_read_signup("sign_up.csv", j); // returns index from prev excel read
    ...
    if(validans_yesorno(answer1) == 1) { // 1 for yes
        ...
        if(validans_yesorno(answer2) == 1) {
            delete("sign_up.csv"); // study this func
            printf("Slots to sign-up have freed. Proceed to j.\n");
            system("clear");
            //usleep(2000000);
            jog_squad_members();
        }
...
void log_in() {
    ...
    printf("Enter # to proceed. Press b/B to Log-in or Sign-up. Press h/H to go back to Home Page.\n\n");
    int log_upper = disp_joggerUN("sign_up.csv") - 1;
...
void sign_up() {
    ...
    store_signup("sign_up.csv", jogger);
...
void store_jogentry(const char *csv_file, struct jogger_info j[MAX_MEMBERS]) {
    ...
    int curr_arr_size = csvrow_read_signup("sign_up.csv", j); // check size from sign up data
...
void o_signup(const char *csv_file, struct jogger_info z[MAX_MEMBERS], int curr_arr_size) {    //the arguments here shoud be
...
//	int curr_arr_size = csvrow_read_signup("sign_up.csv", j); // check size from sign up data
    ...
    fprintf(ptr, "Username,*Password,MALE/FEMALE,*Age (yrs),*Height (cm),*Weight (kg),Current Best Pace (min/mi)\n"); // Write header
    for (int i = 0; i < curr_arr_size; i++) { // wrong limit
         fprintf(ptr, "%s,%s,%s,%d,%.2lf,%.2lf,%.2lf\n", z[i].name, z[i].password, z[i].gender, z[i].age, z[i].height,  z[i].weight, z[i].pace); // print in the csv file
    }
    fclose(ptr);
}

void mod_profile(struct jogger_info j, struct jogger_info z[MAX_MEMBERS], int jogger_index){
	system("clear");
	printf("Please enter your new details: \n");
	int curr_arr_size = csvrow_read_signup("sign_up.csv", z); // check size from sign up data
	int age2 = 0.0; 
	double height2= 0.0, weight2 = 0.0;
	//  receiving age input from the user
		...
		z[jogger_index].age = age2;
		z[jogger_index].height = height2;
		z[jogger_index].weight = weight2;
		
		/*printf("%d\n", j[jogger_index].age);
		printf("%.2lf\n", j[jogger_index].height);
		printf("%.2lf\n", j[jogger_index].weight);
		printf("%d", jogger_index);*/
		o_signup("sign_up.csv", z, curr_arr_size);					//call to change values of the excel file
		
		printf("\nProfile successfully modified.");
		usleep(1800);
		system("clear");
		choices(z, jogger_index);
...
void access_jogger(struct jogger_info j[MAX_MEMBERS], int jogger_index) {
    csvrow_read_signup("sign_up.csv", j); // password purposes
    char password[MAX_STRLN];
	do {
...
void choices (struct jogger_info j[MAX_MEMBERS], int jogger_index) {
    ...
    {
    case 1:
        view_profile(j[jogger_index], j, jogger_index);
        break;
    case 2: 
        log_jogging(j[jogger_index], j, jogger_index);
        break;
    case 3:
		mod_profile(j[jogger_index], j, jogger_index);						//call to change values of the structure
    default:
        break;
    }
}
...
void view_profile(struct jogger_info j, struct jogger_info z[MAX_MEMBERS], int jogger_index) {
    system("clear");
    display_ascii("ascii_art/profile_js.txt", 0);

    printf("%-15s\t %s\n%-15s\t %s\n%-15s\t %d\n%-15s\t %.2lf cm\n%-15s\t %.2lf kg\n%-15s\t %.2lf min/mi\n", 
    "Name: ", j.name, "Gender: ", j.gender, "Age: ",  j.age, "Height: ", j.height, "Weight: ",  j.weight, "Best Pace: ", j.pace);
    
    if(j.pace >= 0.0 && j.pace <= 6.0){
		printf("Status: Beginner Jogger");
	}else if(j.pace >= 7.0  && j.pace <= 10.0){
		printf("Status: Intermediate Jogger");
	}else{
		printf("Status: Advanced Jogger");
    }
    double BMR;
    ...
    struct achieved achievements[] = {
        ...
        {10, "Quick Jog"},
        {20, "Steady Runner"},
        {30, "Half-Hour Hustler"},
        {60, "Hour Hero"},
        {300, "Calorie Crusher"},
        {500, "Energetic Explorer "},
        {1000, "Calorie Conqueror"},
        {2000, "Fuel Burner"},
        {-1, " "}
    };
...
int total_count = 0;
    ...
    for (int k = 0; k < 4; k++) {
        if (j.distance >= achievements[k].goal) {
            distance_count++;
        }
    }

    for (int k = 4; k < 8; k++) {
        if (j.duration >= achievements[k].goal) {
            duration_count++;
        }
    }

    for (int k = 8; k < 12; k++) { 
        if (j.calories >= achievements[k].goal) {
            calorie_count++;
        }
    }
    
    total_count = distance_count + duration_count + calorie_count;
	...
	printf("\n\n\t\t\t\t\t\tRECOMMENDED ROUTINE: ");
	jogging_recommend(user_pace);
	
	printf("\n\nEnter to proceed: H = Back to Profile | B = Log-Out\n");
    int log_upper = 0;
	...
	//	home_screen();
		...
		display_ascii("ascii_art/log_out.txt", 0);
		usleep(1500);
        jog_squad_members();
    }
}
...
int recommendations(double tdee){
	...
	if(tdee >= 1100 && tdee <= 1200){
		file = fopen("1200 Cal Plan.csv", "r");
	}else if(tdee > 1200 && tdee <= 1500){
		file = fopen("1500 Cal Plan.csv", "r");
	}else{
		file = fopen("2000 Cal Plan.csv", "r");
    }
...
double jogging_recommend(double user_pace){
	...
	if(user_pace >= 0.0 && user_pace <= 6.0){
		file = fopen("beginner rec.csv", "r");
	}else if(user_pace >= 7.0  && user_pace <= 10.0){
		file = fopen("intermediate rec.csv", "r");
	}else{
		file = fopen("advanced rec.csv", "r");
    }
   ...
   int line_count = 1; // Initialize a counter variable to keep track of the line number

while (fgets(line, sizeof(line), file)) {
    token = strtok(line, delimiter);

    // Print the first token (first column) separately
    //printf("\t\t%s\t", token);
    
    // Move to the next token (second column) and print the rest of the tokens in list format
    token = strtok(NULL, delimiter);
    printf("DAY %d: ", line_count); // Print the line number
    while (token != NULL) {
        printf("\t\t%-10s\t\n", token);
        printf("\n");
        token = strtok(NULL, delimiter);
...
void log_jogging(struct jogger_info j, struct jogger_info z[MAX_MEMBERS], int jogger_index) {
    ...
    csvrow_read_signup("sign_up.csv", z); 
    csvrow_read_jogentry("jog_entry.csv", z);
    ...
    z[jogger_index].entry += 1;
    z[jogger_index].distance += miles;
    z[jogger_index].duration += minutes;
    z[jogger_index].calories +=  (7 * 3.5 * z[jogger_index].weight / 200) * minutes;
    
    double jogger_cal = round((7 * 3.5 * z[jogger_index].weight / 200) * minutes);
    z[jogger_index].calories +=  jogger_cal;
...
// minor formatting issue
    ...
    double session_pace = z[jogger_index].duration / z[jogger_index].distance;

    // over writing in sign up info the best pace CELL SPECIFIC
    if(session_pace > z[jogger_index].pace) {
        z[jogger_index].pace = session_pace;
        printf("You have a new all-time best pace of %.2lf min/mi :O\n", session_pace);
    }

    // TABLE CODE HERE
    overwriterow_signup(z, "sign_up.csv", jogger_index);
    clear_csv("jog_entry.csv");
    store_jogentry("jog_entry.csv", z);
    
    printf("\n\nEnter to proceed: H = Back to Profile Menu | B = Log-Out\n");
    int log_upper = 0;
...
void indiv_stats() {
    system("clear");
    printf("Enter # whose individual statistics you want to see\n\n");
    
    int indiv_upper = disp_joggerUN("sign_up.csv") - 1;
    ...
    struct jogger_info jogger[MAX_MEMBERS];
    csvrow_read_signup("sign_up.csv", jogger); // array storing
    csvrow_read_jogentry("jog_entry.csv", jogger); // array storing

    system("clear");
    display_ascii("ascii_art/stats_js.txt", 0);
        ...
        printf("\n\n");
         ...
         printf("\n\nEnter to proceed: H = Stats Menu | B = Log-Out\n");
		int log_upper = 0;
    
		char log_opt[MAX_STRLN];
		int opt2 = interface_opt(log_opt, log_upper);
...
void team_stats(const char *csv_signup, const char *csv_jogentry) {	
    ...
    FILE *signup_file = fopen("sign_up.csv", "r");
    FILE *jogentry_file = fopen("jog_entry.csv", "r");
    ...
    printf("+----------------------------------------------------------------------------------------------------------------+\n");
    printf("|   %-10s\t |\t%-5s\t |\t%-5s\t |\t%-5s\t |\n", "Name", "Total Distance (mi)", "Total Time (mins) ", "Total Calories Burned");
    printf("+----------------------------------------------------------------------------------------------------------------+\n");
...
void view_stats() {
	...
	case 2:
		team_stats("sign_up.csv", "jog_entry.csv");
		break;
...
void delete (const char *csv_file) {
  ...
  int upper = disp_joggerUN("sign_up.csv") - 1; // array size in a way
  ...
  do {
    ...
    delete_row("sign_up.csv", i_opt + 1);
    delete_row("jog_entry.csv", i_opt + 1);
}
...
void hall_of_achievements() {
    ...
    printf( "Enter # to proceed. Press b/B to Log-in or Sign-up. Press h/H to go back to Home Page.\n"
        "\nWhich hall do you want to check?"
        "\n1. Distance Achievements"
        "\n2. Consistency Achievements"
        "\n3. Calories Burned Achievements\n");

    char achieve_opt[MAX_STRLN];
    int opt = interface_opt(achieve_opt, 2);
    ...
    case 1:
        distance_achieved(jogger);
        break;

    case 2:
        duration_achieved(jogger);
        break;

    case 3:
        calories_achieved(jogger);
        break;
...
void distance_achieved(struct jogger_info j[MAX_MEMBERS]) {
    system("clear");
    display_ascii("ascii_art/distance.txt", 0);

    csvrow_read_signup("sign_up.csv", j);
    csvrow_read_jogentry("jog_entry.csv", j);
    ...
    // Print the members and their achievements
    for (int i = 0; i < MAX_MEMBERS; i++) {
        if (j[i].distance > 0) { // Check to avoid printing uninitialized members
            //printf("| %-15s ", j[i].name);
...
void duration_achieved(struct jogger_info j[MAX_MEMBERS]){
	system("clear");
	display_ascii("ascii_art/duration.txt", 0);
    ...
    // Print the members and their achievements
    for (int i = 0; i < MAX_MEMBERS; i++) {
        if (j[i].duration > 0) { // Check to avoid printing uninitialized members
...
void calories_achieved(struct jogger_info j[MAX_MEMBERS]){
	system("clear");
	display_ascii("ascii_art/burned.txt", 0);
	csvrow_read_signup("sign_up.csv", j);
    csvrow_read_jogentry("jog_entry.csv", j);
  ...
  printf("+-----------------+-----------------+-----------------+-----------------+\n");
    ...
    // Print the members and their achievements
    for (int i = 0; i < MAX_MEMBERS; i++) {
        if (j[i].calories > 0) { // Check to avoid printing uninitialized members
            //printf("| %-15s ", j[i].name);
